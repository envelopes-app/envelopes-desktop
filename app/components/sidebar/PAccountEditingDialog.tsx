...
export interface PAccountEditingDialogProps { 
	onUpdateAccount: (account:IAccount, currentBalance:number)=>void;
}

export class PAccountEditingDialog extends React.Component<PAccountEditingDialogProps, {showModal:boolean, account?:IAccount}> {
	...
	private save():void {
		...
		this.props.onUpdateAccount(account, currentBalance);
			...
			<Modal show={this.state.showModal} onHide={this.close} backdrop="static" keyboard={false}>
				...
				<Modal.Footer>
					...
					<Button onClick={this.save}>
						Save Account&nbsp;<Glyphicon glyph="ok-circle" />
					</Button>
				</Modal.Footer>
			</Modal>
		);
