...
export class PayeeHelper {
	...
	public getPersistenceQueries(budgetId:string,
		...
		if(changedEntities.payees) {

			_.forEach(changedEntities.payees, function(changedEntity:budgetEntities.IPayee) {
				...
				queriesList.push(budgetQueries.PayeeQueries.insertDatabaseObject(changedEntity));
			});
		}
