...
module ynab.queries {
    ...
    export class BudgetDateQueries {
        ...
        public static getFirstAndLastBudgetMonthQuery(budgetVersionId:string):ynab.interfaces.adapters.IDatabaseQuery {
...
WITH e_first_last_budgeted_month AS (
	...
	WHERE budgetVersionId = ?1
		AND isTombstone = 0
		AND COALESCE(source,'') IN (${ynab.queries.TransactionQueries.TransactionSourcesINClause})
)
